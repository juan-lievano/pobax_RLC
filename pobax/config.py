...
class DQNHyperparams(Tap):
    algo: str = 'dqn'  # used by probe pipeline to identify algorithm
    env: str = 'compass_world_8'
    num_envs: int = 4
    gamma: float = 0.99  # will be replaced if env has gamma property.

    memoryless: bool = False  # False = DRQN (GRU), True = DQN (FF)
    action_concat: bool = False  # Do we concatenate actions to our observation?

    # Sweepable hyperparameters
    lr: list[float] = [2.5e-4]
    epsilon_finish: list[float] = [0.05]
