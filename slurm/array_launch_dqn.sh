#!/usr/bin/env bash
#SBATCH --job-name=pobax_dqn_grid
...
GRID_FILE="/nas/ucb/juanlievano/pobax_RLC/slurm/grid_dqn.tsv"

if [[ ! -f "$GRID_FILE" ]]; then
  echo "ERROR: grid file not found at $GRID_FILE"
  echo "Did you run: python slurm/make_grid_dqn.py  (from repo root)?"
  exit 1
fi
...
bash /nas/ucb/juanlievano/pobax_RLC/slurm/run_one_dqn.sh
